% function [Result_a, Result_b, STAT_QC1, STAT_QC2] = Q_CoNoP_2by2()

% Condition 3: Communicable condition; don't know others position
% Multi-agnent Q-learning for communication
...
for epi = 1:EPI
    ...
    while ~epi_end
        ...
        %% take a^p from (s^p, s^c) using policy derived from Q^p
...
%         [r1, r2, epi_end] = calc_rewards_cost(new_sp1, new_sp2, GOAL_POS, new_ac1, new_ac2);
        
        %% communicate a'^c form s'^p
        ...
        if ~epi_end
            ...
            delta_c1 = delta_c1 + gamma_c * max(Qc1(new_sp1, :));
            delta_c2 = delta_c2 + gamma_c * max(Qc2(new_sp2, :));
        end
%     if sp1 == 1 || sp2 == 1
%       fprintf('sp1: %d, sp2: %d\n', sp1, sp2)
%     end
...
% save('Q_AverageReward_conop_2by2.mat','stat_r')
%% plot reward (mean)
