function [Result_a, Result_b] = Q_NoCoP_2by2()

% Condition 2: observable conditions; knowing others position
% Q-learning agents
...
% init Q^p(sp1,sp2,a^p)
Qp1 = 0.001*rand(NUM_POS,NUM_POS,NUM_A); 
Qp2 = 0.001*rand(NUM_POS,NUM_POS,NUM_A); % Q1(4,:) = 0.1; % bias on up action
...
for epi = 1:EPI
    ...
    while ~epi_end
        ...
        q1 = squeeze(Qp1(sp1,sp2,:));
        q2 = squeeze(Qp2(sp2,sp1,:));
        ...
        if ~epi_end
            new_q1 = squeeze(Qp1(new_sp1,new_sp2,:));
            new_q2 = squeeze(Qp2(new_sp2,new_sp1,:));

            delta_p1 = delta_p1 + gamma_p * max(new_q1);
            delta_p2 = delta_p2 + gamma_p * max(new_q2);
        end
         
        Qp1(sp1,sp2,ap1) = Qp1(sp1,sp2,ap1) + alpha_p * delta_p1;
        Qp2(sp2,sp1,ap2) = Qp2(sp2,sp1,ap2) + alpha_p * delta_p2;
        %%
        sp1 = new_sp1;
        sp2 = new_sp2;
    end
...
% save('Q_AverageReward_nocop_2by2.mat','stat_r')
%% plot reward (mean)
